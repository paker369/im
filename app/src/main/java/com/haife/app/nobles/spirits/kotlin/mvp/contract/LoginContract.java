...
public interface LoginContract {
    ...
    interface View extends IView {
        void loginbyPwdSuccess(LoginBean data);

        void registerUserSuccess(LoginBean data);
    }

    //Model层定义接口,外部只需关心Model返回的数据,无需关心内部细节,即是否使用缓存
    interface Model extends IModel {
        Observable<BaseResponse<LoginBean>> loginbyPwd (RequestBody body);

        Observable<BaseResponse<LoginBean>> registerUser (RequestBody body);
    }
}
